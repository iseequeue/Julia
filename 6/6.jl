...
# ╔═╡ 7ad782c0-593b-11ed-207e-e5357f59d5ce
begin
	using Plots
	using LinearAlgebra
	using LaTeXStrings
...
# ╔═╡ 32c3223a-0b3e-4602-89d5-c84392340cad
begin
...
"""
function itproot(f, x₁, x₂, z, K; xtol=eps(), ftol=eps(), κ₁=0.1, κ₂=2, n₀=1)
    if x₁ > x₂; x₁, x₂ = x₂, x₁; end
    y₁, y₂ = f(x₁, z, K), f(x₂, z, K)
...
# ╔═╡ 8c5b01e8-5ba2-4143-9bdf-1de66284bac8
begin

	function F(G, z, K)
		f=0.0
		if (length(z)!=length(K))
			throw(DomainError("different sizes of z&K"))
		end
		for i in 1:length(K)
			f+=z[i]*(K[i]-1)/(G*(K[i]-1)+1)
		end
		return f
		end

	function rachford_rice_solve(z, K)
		x1, x2= 1/(1-maximum(K)), 1/(1-minimum(K))
		return itproot(F, x1, x2, z, K; xtol=2e-10, n₀=0)
	end
end

# ╔═╡ 464cac34-28d8-4642-b5ef-9d04c227ff20
let
	z=[0.2463, 0.2208, 0.2208, 0.3121]
	K=[40, 25, 0.6, 0.005]
	
	sol = rachford_rice_solve(z, K)

	println(sol)

	
	
	G = range(-0.5, 1; length=2000)
	
	plt = plot(; ylim=(-1, 1), xlim=(-0.5, 1),  xlabel="G",   ylabel="F",
	    lw=2,
	)
...
# ╔═╡ 81766ff7-3996-4b4e-82a8-1753ecc2ce4f
let
	z=[0.9, 0.1]
	K=[1.5, 0.01]
