...
# ╔═╡ bd4ef080-7a46-11ed-150a-a30e3aae2983
begin
	using ForwardDiff
	using LinearAlgebra
	using StructArrays
...
# ╔═╡ 199fa70b-c8e0-4846-bcc6-eb8975fef01a
begin
...
# ╔═╡ 8aba7b65-a12d-43ee-9df2-80e57291cdfb
begin
	GAS_CONSTANT_SI = 8.31446261815324
...
# ╔═╡ 5718b921-cc75-4a42-88dd-abe52391cd27
begin
...
# ╔═╡ 7972ed60-0b52-43bd-a972-2bda852fd388
begin
...
# ╔═╡ 12baa3cb-107b-4f1b-b028-b85d9d8acb40
begin
...
# ╔═╡ 2962e38c-350a-4852-bfe0-6d2982e51348
let
	x = zeros(0)
	append!( x, 1.9 )
	append!( x, 2.0 )
...
# ╔═╡ 85650d21-44a1-45fb-a3f0-0b0ae7f0f758
let
	c1 = VanDerWaalsComponent(2.283 / 10, 0.04278 / 1000, 4.5992e6, 190.56 *GAS_CONSTANT_SI)
	c2 = VanDerWaalsComponent(5.562 / 10, 0.0638 / 1000, 4.88e6, 305 *GAS_CONSTANT_SI)
	c3 = VanDerWaalsComponent(8.779 / 10, 0.08445 / 1000, 4.26e6, 370 *GAS_CONSTANT_SI)
	c4 = VanDerWaalsComponent(14.66 / 10, 0.1226 / 1000, 3.80e6, 425 *GAS_CONSTANT_SI)
	c5 = VanDerWaalsComponent(19.26 / 10, 0.146 / 1000, 3.38e6, 470 *GAS_CONSTANT_SI)
	c6 = VanDerWaalsComponent(24.71 / 10, 0.1735 / 1000, 3.03e6, 508 *GAS_CONSTANT_SI)
	c7 = VanDerWaalsComponent(1.370 / 10, 0.0387 / 1000, 3.39e6, 126 * GAS_CONSTANT_SI)
	
	mixture = VanDerWaalsMixture([c1, c2, c3, c4, c5, c6, c7])
	molfrac = [0.9430, 0.0270, 0.0074, 0.0049, 0.0027, 0.010, 0.0140]
	
	pressures = range(1e5, 70e5; length=100)
	temperatures = range(150, 250; length=350)
	...
	for P in pressures, T in temperatures
	    RT = T * GAS_CONSTANT_SI
	    Kinit = michelsen.(mixture.components, P, RT)
	    results = [
	        stability(mixture, molfrac, P, RT, molfrac ./ Kinit, :gas, :liquid)
	        stability(mixture, molfrac, P, RT, molfrac .* Kinit, :liquid, :gas)
	    ]

		if (Int(results[1].converged)*Int(results[2].converged)==0)
			append!(errors_t, T)
			append!(errors_p, P)
		elseif (results[1].tpd>0 && results[2].tpd>0)
			append!(stable_t, T)
			append!(stable_p, P)
		elseif (results[1].tpd<0 || results[2].tpd<0)
			append!(nonstable_t, T)
			append!(nonstable_p, P)
		end
		
		write(io, join(Any[T, P, Int(results[1].converged), results[1].tpd, 
	...
	scatter(nonstable_t, nonstable_p;color="yellow",markerstrokewidth=0, markersize=4.0, label="nonstable", legend=:bottomright, xlabel=L"T", ylabel=L"P")
	scatter!(stable_t, stable_p;color="blue",markerstrokewidth=0, markersize=4.0,label="stable")

	scatter!(errors_t, errors_p; color="red", markerstrokewidth=0, markersize=4.0, label="errors")
end

# ╔═╡ 7752d4f8-c618-48a2-95ca-eea609528d3a


# ╔═╡ ec3adf8d-d8c2-4228-9c9b-11217b2a9fce


# ╔═╡ 919c8c36-b0bb-412e-b230-0aa25ee06c1c


# ╔═╡ 0eede906-75ea-476d-ab94-c780b098ecea


# ╔═╡ b3b59e2e-94ca-4ad2-9bea-70f32bbb3fcd


# ╔═╡ 0251c5fa-f2a4-4591-b813-e7aafde4d790


# ╔═╡ cff8ad20-223d-4c7b-8e8c-9caa791b4259


# ╔═╡ 8ef2f59f-67ca-4e93-bb2b-85693eb45d3f
...
julia_version = "1.8.1"
manifest_format = "2.0"
project_hash = "ea9a6c35f441fffe119db605b0b6c3b2d20ceaa6"
