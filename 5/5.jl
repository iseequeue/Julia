...
# ╔═╡ 4c19621e-4393-4a42-a9a1-d74b07d3ffb6
begin
	using LinearAlgebra
	using Plots
	using LaTeXStrings
...
# ╔═╡ aa7a4057-e83a-4e32-889b-08d80595f9cb
begin
...
# ╔═╡ 71c9b852-e619-4604-bd4e-d5a52acac2c4
let 
...
# ╔═╡ 44812028-a052-43bf-ae11-7514c1b8f82c
begin
...
let
	y=[0.0010619974947523314,
6.380592467358781e-5,
6.380592467358781e-5,
3.946858164738387e-6,
1.706567423154226e-6,
2.4603455894833814e-7,
1.5367083605610787e-8,
5.652646128950778e-9,
9.6028518470348e-10,
7.027756154798226e-11,
2.299005430472789e-11]
	x=[5,
9,
9,
17,
...
397]
	...
	scatter(log.(x), log.(y); label="данные", legend=:bottomright, xlabel=L"ln(nodes)", ylabel=L"ln(Difference)")

	x=[2,3,4,5,6]
	plot!(x, x.*(-4); label="approx y=-4x", legend=:bottomright, xlabel=L"ln(nodes)",ylabel=L"ln(Difference)") #4 порядок сходимости
end
